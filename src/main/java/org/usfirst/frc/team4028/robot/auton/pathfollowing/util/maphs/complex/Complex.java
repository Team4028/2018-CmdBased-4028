...
public class Complex extends Number 
    ...
    public Complex(double first, double second, boolean polarQ){
		if (polarQ) {
			this.real = first * Math.cos(second);
			this.imag = first * Math.sin(second);
		} else {
			this.real = first;
			this.imag = second;
		}
	...
	// log (z) = log (r*e^iTheta) = log(r) + i*Theta
	public static Complex log(Complex z){
		return new Complex(StrictMath.log(z.real), arg(z));				
	}
